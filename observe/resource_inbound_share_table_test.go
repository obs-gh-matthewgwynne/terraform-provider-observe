...
var (
	...
	testInboundTableTypes = getenv("TEST_INBOUND_TABLE_TYPES", "COMPREHENSIVE_TYPES_TEST") // Table with various data types for field mapping tests
)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: Create with minimal configuration
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("observe_inbound_share_table.test", "dataset_label", randomPrefix),
					resource.TestCheckResourceAttr("observe_inbound_share_table.test", "description", ""),
